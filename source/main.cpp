...
static const char load_dolphin[] ATTRIBUTE_ALIGN(32) = "/dev/dolphin";
...
int main()
{
	...
	if (fd >= 0)
	{
		...
		if(disc_check & 0x2)
		{
			...
			else if(Disc_IsWii() == 0)
			{
				...
				/* gameconfig */
				f = fopen("sd:/NetplayV3/gc.txt", "rb");
				if(f != NULL)
				{
					fseek(f, 0, SEEK_END);
					fsize = ftell(f);
				...
				/* gct */
				f = fopen("sd:/NetplayV3/RSBE01.GCT", "rb");
				if(f != NULL)
				{
					gprintf("Opened gct\n");
					fseek(f, 0, SEEK_END);
	...
	else if (fd < 0)
	{
		...
		u32 disc_check = 0;
		WDVD_GetCoverStatus(&disc_check);
		gprintf("This launcher is only meant for Dolphin\n");
		/* Fail, init chan launching */
		WII_Initialize();
		/* goto HBC */
		WII_LaunchTitle(HBC_LULZ);
